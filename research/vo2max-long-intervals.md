...
## Terminology — "Norwegian Method" is ambiguous, pin it down before using

Two distinct training paradigms are both routinely called "Norwegian" in
the popular press and on social media. The 4×4-min protocol that
amateurs typically ask about is the **first** of these, NOT the second.

| Name in popular use | Actual content | Originator | Population |
|---------------------|----------------|------------|------------|
| **Norwegian 4×4 / Norwegian HIIT** | Long VO2max intervals: 4 × 4 min @ 90–95 % HRmax with 3 min active recovery @ 70 % HRmax, 3×/week, 8 weeks | Helgerud, Hoff, Wisløff (NTNU 2007) | Healthy adults → competitive endurance |
| **Norwegian Method / Norwegian Double-Threshold** | Sub-LT2 lactate-controlled threshold work, often **twice daily**, lactate 2.0–3.0 mmol/L, very high volume | Marius Bakken; refined by Gjert Ingebrigtsen for Jakob & Filip | Elite middle-distance / marathon |

The two methods share almost nothing structurally: Helgerud's 4×4 is a
classic supramaximal-of-LT2 VO2max stimulus; the double-threshold
method is a sub-LT2 volume strategy that requires repeated lactate
testing. **Mixing the two on the same training calendar is not "the
...
1. **The Norwegian 4×4 is a validated VO2max stimulus.** Helgerud 2007:
   4 × 4 min @ 90–95 % HRmax with 3 min @ 70 % HRmax active recovery,
   3×/week × 8 weeks → **+7.2 % VO2max**. The same study's 15/15 short
   intervals → +5.5 %. Both improved VO2max more than continuous LSD
   work.
2. **Long intervals win on time-above-90 % VO2max.** Frontiers 2024
   (Skovgaard, trained middle-distance runners): 4 × 3 min @ 95 % vVO2max
   spent **≈ 5.5 min above 90 % VO2max**, while 24 × 30 s @ 100 % vVO2max
...
### 1. The Helgerud 4×4 protocol — original spec and result

**Helgerud et al. 2007** (Med. Sci. Sports Exerc.):

| Group | Protocol | Frequency | VO2max gain |
|-------|----------|-----------|-------------|
| 4×4 long | 4 × 4 min @ 90–95 % HRmax, 3 min @ 70 % HRmax active rec | 3×/week × 8 weeks | **+7.2 %** |
| 15/15 short | 47 × (15 s @ 90–95 % HRmax + 15 s @ 70 % HRmax) | 3×/week × 8 weeks | **+5.5 %** |
| Lactate threshold | continuous @ 85 % HRmax | 3×/week × 8 weeks | smaller |
| LSD continuous | continuous @ 70 % HRmax | 3×/week × 8 weeks | smaller |
...
### 2. Time-above-90 % VO2max — long beats intensified short

**Skovgaard et al. 2024 (Frontiers in Sports and Active Living)**:

| Format | Intensity | Recovery | Time > 90 % VO2max |
|--------|-----------|----------|---------------------|
| 4 × 3 min "long" | 95 % vVO2max | 3 min @ 50 % vVO2max | **328 ± 147 s** (≈ 5.5 min) |
| 24 × 30 s "intensified short" | 100 % vVO2max | 30 s @ 55 % vVO2max | **201 ± 268 s** (≈ 3.4 min) |

The short-interval format ran faster but failed to accumulate
time-above-90 % VO2max because the short rests kept work-VO2 and
rest-VO2 at "approximately 78 % VO2max" — never letting VO2 climb
into the stimulus band. **Source:**
[Frontiers 2024](https://pmc.ncbi.nlm.nih.gov/articles/PMC11743937/).

This result is consistent with the older Tjelta / Helgerud line of
work: long intervals (3–4 min) at 90–95 % HRmax are a reliable
...
Expected time-above-90 % VO2max comparable to Helgerud 4×4 on the run.
...
The Bakken / Ingebrigtsen double-threshold method is a different
question and not interchangeable with 4×4. Its core idea is **sub-LT2
threshold work twice per day** at lactate 2.0–3.0 mmol/L, accumulating
very high weekly volume without the systemic fatigue of single
above-LT2 sessions. Key practical preconditions:

- **Lactate testing in every session** — without portable lactate
  meters and known LT2, the prescription is uncalibratable.
...
| Run Pace Pal — n.d. | 4×4 VO2max intervals: Why They Don't Work for Most Runners | [runpacepal.com](https://runpacepal.com/blog/why-standard-4x4-vo2max-intervals-wont-work) | Coach-practice caveats for the running implementation |
...
2. **Hill-repeats vs flat 4×4.** When a race-specific hill-repeat
   ...
   hill-repeats and flat 4×4" plan within a single block would
   produce extra VO2max gain over hill-repeats alone — no controlled
   study found.
3. **Combination with strength concurrent.** The interference logic
   in [concurrent-training-interference.md](concurrent-training-interference.md)
   applies to 4×4 the same as to any VO2max stimulus. The 6+h
   spacing rule and the legs/upper-body separation hold.
4. **HRmax vs vVO2max as the anchor.** Helgerud uses % HRmax;
   Skovgaard uses % vVO2max. They are close but not identical (HRmax
   tends to be reached at slightly lower velocity than VO2max in
   trained runners). For an athlete with a well-anchored LTHR but
   no recent vVO2max test, % HRmax is the more practical control;
   for a track-tested vVO2max athlete, % vVO2max is more precise.
